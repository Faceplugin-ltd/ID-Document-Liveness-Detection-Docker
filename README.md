...
> **Fastest:** `docker pull faceplugin/document-liveness:latest` → run → copy `FPMC1.…` → activate.
...
- [ ] Download and run the appropriate Docker image from [FacePlugin Docker Hub](https://hub.docker.com/r/faceplugin/document-liveness). [See Option A for details](#option-a--docker-hub-no-drive-download).
...
| Feature                                  | Supported |
| ---------------------------------------- | --------- |
| Document authenticity / security checks  | ✓         |
...
| 3    | You do not need a license to start the API the first time. Copy the machine code (`FPMC1.…`) from the logs or `GET /api/machinecode`. Send it to FacePlugin ([contact](#contact)) to get license key and unlock product endpoints. |
...
docker run -d --name faceplugin-document-liveness \
...
If you are building or running directly from this repository, download the **CPU** package into `./lib/cpu/`. This product is **CPU-only** — there is no `gpu/` package.
...
API: **[http://127.0.0.1:8086](http://127.0.0.1:8086)**
...
<p align="center">
 <img src="assets/screenshots/demo-ui-security.png" alt="FacePlugin ID Document Liveness Linux demo — Security tab with authenticity and anti-spoofing checks" width="900"/>
</p>

<p align="center">
 <img src="assets/screenshots/demo-ui-raw.png" alt="FacePlugin ID Document Liveness Linux demo — Raw JSON API response" width="900"/>
</p>
