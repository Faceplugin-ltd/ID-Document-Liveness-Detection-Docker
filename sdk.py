"""ID Document Liveness Detection — ctypes bindings to libDocSDK.so (lib/cpu/).
...
This product uses DocSDK authenticity / security checks only (OCR / MRZ / Barcode off by default).
"""
...
# Default: authenticity / security only (no OCR / MRZ / Barcode / ImageQuality payload).
_LIVENESS_OPTS = {
    "response": {
        "OCR": "none",
        "MRZ": "none",
...
def _normalize_images(images) -> list:
    """Accept list of b64 strings or ``{image, page_idx}`` dicts."""
    norm = []
    for i, item in enumerate(images or []):
        if isinstance(item, dict):
            ...
            norm.append({"image": _fit_b64(item).decode(), "page_idx": i})
    return norm


def document_liveness(images, rfid: str = "", options: dict | None = None) -> str:
    """Run document authenticity / security checks. Returns DocSDK JSON string."""
    opts = options or dict(_LIVENESS_OPTS)
    # Ensure Authenticity stays on even if caller only overrides part of response.
    if isinstance(opts.get("response"), dict) and "Authenticity" not in opts["response"]:
        opts = {**opts, "response": {**opts["response"], "Authenticity": "normal"}}
