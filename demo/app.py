...
Liveness / authenticity checks only (no OCR / MRZ / Result / Images tabs).
"""
...
def check_liveness(front, back):
    ...
    if lic and not lic.get("authenticity") and not lic.get("licensed"):
        return (
            "**Note:** License unavailable — liveness checks will not run.\n\n"
    ...
    if lic and not lic.get("authenticity"):
        prefix.append(
            ...
            "liveness checks may be empty / not checked."
        )
...
def main() -> None:
    ...
    with gr.Blocks(title="ID Document Liveness Detection Demo") as demo:
        gr.Markdown(
            ...
            "Upload a document image and click **Check Liveness**."
        )
                ...
                with gr.Tabs():
                    with gr.Tab("Liveness"):
                        sec_summary = gr.Markdown(
                            value="*Run Check Liveness to see authenticity checks.*"
                        )
