# ID Document Liveness Detection — Linux SDK + Docker (same repo)
# Native libs are linux/amd64; image runs on any Docker host (Win/Mac/Linux).
# Before build: download Drive folder contents into ./lib/cpu/ (see README)
...
# Listen on product API port 8086 (same as host publish)
ENV PORT=8086
RUN chmod +x ./run.sh \
